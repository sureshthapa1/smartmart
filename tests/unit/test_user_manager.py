...
def test_create_user_duplicate_raises_value_error():
    user_manager.create_user("charlie", "Pass1234", "staff")
    with pytest.raises(ValueError, match="already taken"):
        user_manager.create_user("charlie", "Pass5678", "admin")
...
def test_update_user_username():
    user = user_manager.create_user("dave", "Pass1234", "staff")
    updated = user_manager.update_user(user.id, {"username": "david"})
    assert updated.username == "david"


def test_update_user_role():
    user = user_manager.create_user("eve", "Pass1234", "staff")
    updated = user_manager.update_user(user.id, {"role": "admin"})
    assert updated.role == "admin"


def test_update_user_duplicate_username_raises():
    user_manager.create_user("frank", "Pass1234", "staff")
    user2 = user_manager.create_user("grace", "Pass5678", "staff")
    with pytest.raises(ValueError, match="already taken"):
...
def test_delete_user_removes_user():
    user = user_manager.create_user("judy", "Pass1234", "staff")
    uid = user.id
    # Need a different current_user_id
    admin = user_manager.create_user("admin_user", "Pass5678", "admin")
...
def test_delete_user_self_deletion_raises():
    user = user_manager.create_user("mallory", "Pass1234", "admin")
...
def test_list_users_ordered_by_username():
    user_manager.create_user("zara", "Pass1234", "staff")
    user_manager.create_user("anna", "Pass5678", "staff")
    user_manager.create_user("mike", "Pass9012", "staff")
    users = user_manager.list_users()
...
def test_list_users_returns_all():
    user_manager.create_user("user1", "Pass1234", "staff")
    user_manager.create_user("user2", "Pass5678", "admin")
    users = user_manager.list_users()
    assert len(users) >= 2

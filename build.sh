...
with app.app_context():
    ...
    try:
        with db.engine.connect() as conn:
            ...
            safe_add(conn, "products", "storage_tips",  "TEXT")
            safe_add(conn, "products", "slug",          "VARCHAR(160)")
            safe_add(conn, "products", "is_featured",   "BOOLEAN DEFAULT FALSE")
        ...
        safe_add(conn, "user_permissions", "can_collect_credit", "BOOLEAN DEFAULT false")
        safe_add(conn, "product_variants",  "units_per_parent",  "INTEGER")
